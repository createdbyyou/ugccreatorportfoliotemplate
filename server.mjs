...
http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url.startsWith('/delete-media')) {
    // a slot emptied in the editor: take its file off disk too
    const u = new URL(req.url, 'http://x');
    const key = (u.searchParams.get('key') || '').replace(/[^a-z0-9_-]/gi, '');
    if (!key) { res.writeHead(400); res.end('no key'); return }
    const dir = join(root, 'assets', 'user', 'slots');
    ...
    try {
      for (const f of readdirSync(dir)) {
        if (f === key + extname(f) && /^\.(mp4|mov|webm|jpg|jpeg|png)$/i.test(extname(f))) {
          try { unlinkSync(join(dir, f)); n++ } catch {}
        }
      }
    } catch {}
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(JSON.stringify({ removed: n }));
    return;
  }
  if (req.method === 'POST' && req.url === '/hero-remove') {
    // the hero film comes down; the site falls back to its colour-wash placeholder
    try { rmSync(join(root, 'assets', 'user', 'hero'), { recursive: true, force: true }) } catch {}
